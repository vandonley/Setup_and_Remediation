...
.EXAMPLE
   pccSet-DefaultPowerSettings
.EXAMPLE
   pccSet-DefaultPowerSettings Desktop
.EXAMPLE
   pccSet-DefaultPowerSettings -ComputerType Desktop
.OUTPUTS
...
$ErrorFile = $env:RMMErrorFolder + "\pccSet-DefaultPowerSettings.txt"
...
try {
    if (! (($ComputerType -eq 'Desktop') -or ($ComputerType -eq 'Laptop'))) {
    	...
    	$Return.ComputerType = "Computer type not specified and unable to detect"
