...
param (	
	[switch]$All = $true, # Accept all default check values in one go
	[switch]$Apply = $true, # -Apply will write new checks to configfiles and reload agent
	[string]$ReportMode = 'On', # -ReportMode will report missing checks, but not fail the script
	[switch]$Performance = $true, # Set to $false if you do not want performance checks
	#[switch]$PingCheck = $false, # This is useful on a Fault History report. Otherwise useless.
	[switch]$MSSQL = $true, # Detect SQL servers
	[switch]$SMART = $true, # Enable physical disk check if SMART status is available
	#[switch]$Backup = $true, # Configure a basic backup check if a compatible product is recognized
	[switch]$Antivirus = $false, # Configure an Antivirus check if a compatible product is recognized
	[switch]$LogChecks = $true, # Configure default log checks
	[string]$DriveSpaceCheck = '10%', # Freespace as number+unit, i.e 10%, 5GB or 500MB
	[string]$WinServiceCheck = 'Default',# "All" or "DefaultOnly". 
	[string]$DiskSpaceChange = '25', # percentage as integer
	[string]$ServerInterval = '5', # 5 or 15 minutes
	[string]$PCInterval = '15', # 15 or 30 or 60 minutes
	[string]$DSCHour = '10', # When DSC check should run in whole hours. Minutes not supported by agent.
	[switch]$Reset = $true, # A one-time reset switch that will remove overwrite existing checks in all chosen categories.
	[switch]$Debug = $true,
	[switch]$Verbose = $true,
	[string]$logfile = "c:\temp\log.txt", # A parameter always supplied by MAXfocus. We MUST accept it.
...
# Test PCInterval
If ('15', '30', '60' -contains $PCInterval) {
	Output-Verbose ('Parameter -PCInterval {0} validated OK.' -f $PCInterval)
} Else {
	Output-Host ('ERROR: -PCInterval {0} could not be validated. Use 15, 30, or 60.' -f $PCInterval)
...
If ($DSChour) {
	...
	If (($isNUM) -and ($x2 -ge 0 -and $x2 -lt 24)) {
		...
		Output-Host 'WARNING: Setting value of -DSChour to default of 10.'
		[string]$DSChour = '10'
	}
}
...
function New-MAXfocusCheck (
	...
	Switch ($checktype) {
		...
		'WinServiceCheck' {
			...
			$object.failcount = 2 # How many consecutive failures before check fails
			$object.startpendingok = 1 # Is Startpending OK, 1 0 Yes, 0 = No
			$object.restart = 0 # Restart = 1 (Restart any stopped service as default)
...
$AlwaysMonitorServices = @( # Services that always are to be monitored if present and autorun
	...
	'SrmSvc', # File Server Resource Manager
	'WAS', # Windows Application Server
	'W3SVC', # Internet Information Server
